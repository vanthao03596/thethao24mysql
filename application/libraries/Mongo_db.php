...
Class Mongo_db{
	...
	*/
	 ...
	 * Sets a field to a value
	 *
	 * @usage: $this->mongo_db->where(array('blog_id'=>123))->set_counter('posted', 1)->update('blog_posts');
	 * @usage: $this->mongo_db->where(array('blog_id'=>123))->set_counter(array('posted' => 1, 'time' => time()))->update('blog_posts');
	 */
	public function set_counter($fields, $value = NULL)
	{
		$this->_u('$inc');
		if (is_string($fields))
		{
	...
	public function group_by($collection)
	{

	}

	public function sum($collection = "", $limit)
	{			
		if (empty($collection))
		{
			show_error("In order to retrieve documents from MongoDB, a collection name must be passed", 500);
			...
			$pipeline = array(
			    ...
			    array(
			        '$project' => array(
			            ...
			            'view' => '$view_number',
			            'like' => '$like_number',
			            'follow' => '$follow_number',
			           	'total' => array('$add' => array(
			           		array('$multiply' => array('$like_number', 6)),
			           		array('$multiply' => array('$view_number', 4)), 
			           		array('$multiply' => array('$follow_number', 2))
			           		)
			           ),
			        ),
			    ),
	...
	public function getThousandUser($collection = "", $listId, $date, $skip, $limit){
		if (empty($collection))
		{
			show_error("In order to retrieve documents from MongoDB, a collection name must be passed", 500);
		}
			...
			$pipeline = array(
					array(
						'$match' => array(
							...
							'user_id_live' => array('$in' => $listId),
							),
						),
					array(
						'$group' => array(
							'_id' => '$user_id_live',
							'name' => array('$last' => '$name'),
							'category_id' => array('$last' => '$category_id'),
							'room_id' => array('$last' => '$room_id'),
							'platform_code' => array('$last' => '$platform_code'),
							'is_live' => array('$last' => '$is_live'),
							'created_at' => array('$last' => '$created_at'),
							),
						),	
					array(
						'$sort' => array("is_live" => -1, "created_at" => -1),
						),
					array('$skip' => ($skip < 0) ? 0 : $skip),				
					array('$limit' => ($limit < 1) ? 1 : $limit),
				);
			$data = $coll->aggregate($pipeline);
	...
	public function getUserOffline($collection = "", $cateId, $listId, $listNotId, $skip, $limit){
		if (empty($collection))
		{
			show_error("In order to retrieve documents from MongoDB, a collection name must be passed", 500);
		}
		try
		{
			...
			if (!empty($cateId)) {
				if (!empty($listNotId)) {
					$listId = array_diff($listId, $listNotId);
					...
					$match = array(
						"category_id" => $cateId,
						"is_active" => 1,
						"user_id" => array('$in' => $list)
						);
				}else{
					$match = array(
						"category_id" => $cateId,
						"is_active" => 1,
						"user_id" => array('$in' => $listId)
						);
				}
			}else{
				if (!empty($listNotId)) {
	...
	public function getUserShare($collection = "", $postId){
		if (empty($collection))
		{
			show_error("In order to retrieve documents from MongoDB, a collection name must be passed", 500);
		}
	...
	public function getByListCategory($collection = "", $listCateId, $listId, $listNotId, $skip, $limit){
		if (empty($collection))
		{
			show_error("In order to retrieve documents from MongoDB, a collection name must be passed", 500);
		}
		try
		{
			...
			$pipeline = array(
					array(
						'$match' => array(
							'category_id' => array('$in' => $listCateId),
							'user_id' => array('$in' => $list)
							),
						),
					array(
	...
	public function getTopUserVideo($collection, $listUserId, $skip, $limit){
		if (empty($collection))
		{
			show_error("In order to retrieve documents from MongoDB, a collection name must be passed", 500);
		}
		try
		{
			...
			$pipeline = array(
				...
				array(
					'$group' => array(
						'_id' => '$user_id',
						'total_view' => array('$sum' => '$view_number'),
						'total' => array('$sum' => 1)
						),
					),
				array(
					'$sort' => array("total_view" => -1, 'total' => -1),
					),
				array('$skip' => ($skip < 0) ? 0 : $skip),				
				array('$limit' => ($limit < 1) ? 1 : $limit),	
			);
	...
	public function getTopEvent($collection = "", $idTop1, $idTop2, $idTop3, $skip, $limit){
		if (empty($collection))
		{
			show_error("In order to retrieve documents from MongoDB, a collection name must be passed", 500);
		}
		try
		{
			...
			$pipeline = array(
				...
				// 	'$project' => array(
				// 		'user_id' => '$user_id',
				// 		'top_1' => array('$item_id' => $idTop1),
				// 		'top_2' => array('$item_id' => $idTop2),
				// 		'top_3' => array('$item_id' => $idTop3),
				// 		//'total' => array('$multiply' => array('$number_item', '$price_item'))
				// 		)
				// 	),
				array(
					'$group' => array(
						'_id' => '$user_id',
						'item_id' => array('$last' => '$item_id'),
	...
	public function getListStreamOfMoth($listId, $startDate, $endDate, $skip, $limit){
	...
	public function getTotalStreamOfMoth($listId, $startDate, $endDate){
		$collection = 'live_stream';
		if (empty($collection))
		{
			show_error("In order to retrieve documents from MongoDB, a collection name must be passed", 500);
